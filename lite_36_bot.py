...
from emoji import emojize # 46. для работы со смайлами импортируем функцию emojize из библиотеки emoji 
...
def greet_user(update, context): # 10. cоздаем функцию для приветсятвия пользователя, при нажатии /start, аргументы update, context в данном случае const
    ...
    smile = get_smile() # добавим смайлы в приветствие
    update.message.reply_text(f'Привет, Пользователь {smile}!') # 12. ответить пользователю
...
def talk_to_me(update, context): # 20. создаем функцию для работы с текстовыми сообщениями пользователя
    ...
    smile = get_smile() # добавим смайлы
    update.message.reply_text(f'{text} {smile}') # 23. Отвечаем пользователю

def get_smile(): # 47. создадим простую функцию без аргументов для подбора смайликов из заданного списка
    smile = choice(settings.USER_EMOJI) # в файл settings.py поместим список со смайликами и с помощью модуля choice берем любой рандомно
    return emojize(smile, language='alias') # c помощью emojize преобразуем текстовый формат смайла в графический, это и будет результатом функции
