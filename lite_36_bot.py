...
def my_planet(update, context): # 26(далее по функции) создадим функцию для команды "planet", которая возвращает пользователю созвездие, в котором находится выбранная им планета
    try:
        ...
        planet_atr = getattr(ephem, user_planet) #(заменяет ephem.Mars, ephem.Pluton) метод getattr позволяет нам получить значение атрибута(в данном случае название планеты) из объекта(класса) ephem
...
def main(): # 2. создаем функцию без аргументов для тела бота
    ...
    dp.add_handler(MessageHandler(Filters.text, talk_to_me, my_planet)) # 19. связываем обработчик сообщений с диспетчером, для вызова функции  при поступлении запроса пользователя, бот будет принимать только текстовые сообщения (прописываем в параметрах с помощью библиотеки Filters)
