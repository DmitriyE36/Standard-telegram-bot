...
def play_cities(update, context): # 33. Создаем фунцию для игры в города при вызове команды \cities
    ...
    if user_city in cities:
        ...
        bot_city = [ city for city in cities if city.lower().startswith(user_city[-1])]
        logger.debug(bot_city)
        update.message.reply_text(f'{bot_city[0]}, Ваш ход')
...
def main(): # 2. создаем функцию без аргументов для тела бота
    ...
    dp.add_handler(MessageHandler(Filters.text, talk_to_me, my_planet, user_wordcount, user_full_moon, play_cities)) # 19. связываем обработчик сообщений с диспетчером, для вызова функции при поступлении запроса пользователя, бот будет принимать только текстовые сообщения (прописываем в параметрах с помощью библиотеки Filters)
